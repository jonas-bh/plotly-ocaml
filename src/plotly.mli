...
module Marker : sig
  ...
  (** Set a single color for all data points.
      ...
      The color may be specified by its color name (e.g., ["LightBlue"]), hex
      code (e.g., ["#ADD8E6"]), or RGB(A) string (e.g.,
      ["rgba(173, 216, 230, 1)"]). The latter allows for specifying
      transparency. *)

  val colors : string array -> Attribute.t list
  (** Set the colors for each of the data points, such that each color
      corresponds to a data point.

      The colors may be specified by their color name (e.g., ["LightBlue"]), hex
      codes (e.g., ["#ADD8E6"]), or RGB(A) strings (e.g.,
      ["rgba(173, 216, 230, 1)"]). The latter allows for specifying
      transparency. *)
...
module Layout : sig
  ...
  module Axis : sig
    ...
    val axis_title :
      (string * Base.Value.value) list -> string * Base.Value.value
  ...
  module Font : sig
    ...
    (** HTML font family - the typeface that will be applied by the web browser.
        The web browser can only apply a font if it is available on the system
        where it runs. Provide multiple font families, separated by commas, to
        indicate the order in which to apply fonts if they aren't available. *)
  ...
  (** Set the global font properties for the graph.
      ...
      Note that fonts used in traces and other layout components inherit from
      the global font. *)
