...
module Type = struct
  ...
  let rec eq : type a b. a t -> b t -> (a, b) eq option =
   fun a b ->
    match (a, b) with
    | Float, Float -> Some Eq
    ...
    | Array a, Array b -> ( match eq a b with Some Eq -> Some Eq | _ -> None)
    | _ -> None
...
module Value = struct
  ...
  let rec to_json v : Ezjsonm.value =
    ...
    | Value (Array ty, xs) ->
        `A (List.map (fun x -> to_json (Value (ty, x))) @@ Array.to_list xs)
        ...
        | v :: vs -> (
            ...
            | Value (ty, _) ->
                let rec check acc = function
                  | [] ->
                      Some
                        (Value (Type.Array ty, Array.of_list @@ List.rev acc))
                  | v' :: vs -> (
                      match v' with
                      | Value (ty', v') -> (
...
module Attributes = struct
  ...
  let to_json xs = `O (List.map (fun (k, v) -> (k, Value.to_json v)) xs)
