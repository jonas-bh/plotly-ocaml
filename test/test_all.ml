...
let get_filename figure : string =
  ...
  | Some title ->
      let sanitized =
        String.map (function ' ' | '/' | '\\' | ':' -> '_' | c -> c) title
      in
      sanitized
...
let test_json_figure figure =
  ...
  let ref_path =
    ...
    if Sys.file_exists p1 then p1 else if Sys.file_exists p2 then p2 else p1
  in
...
let test_python_backend figure =
  ...
  try
    ...
    if Sys.file_exists ref_path then begin
      ...
      Printf.printf
        "  ? %s - No Python reference (run: dune exec \
         test/generate_python_references.exe)\n"
        filename;
      false
    end
  with e ->
...
let check_unique_titles () =
  ...
  | dups ->
      ...
      Printf.printf
        "The following titles appear more than once in Demo.figures:\n";
...
let run_tests () =
  ...
  let python_results =
    try List.map test_python_backend Demo.figures
    with e ->
